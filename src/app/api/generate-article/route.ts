...
一、Markdown标题格式（重要）
- 主标题：使用 # 标题格式，如 # 主标题内容
- 一级标题：使用 ## 标题格式，如 ## 第一个主题
- 二级标题：使用 ### 标题格式，如 ### 小节标题
- 文章必须包含2-4个一级标题 ## 作为主要章节
- 每个一级标题下必须有实质性内容（至少2-3个段落）
- 结尾必须包含 ## 总结建议 或 ## 结尾 作为最后章节

二、图片分布位置（重要）
${imageSource === 'ai' && imageCount > 0 ? `- 必须生成 ${imageCount} 张图片，均匀分布在：
  - 开头：第1张图片 {{IMAGE_1}} 在前两个一级标题之间
  - 中间：第2张图片 ${imageCount > 2 ? `{{IMAGE_2}}` : '{{IMAGE_2替代文字段落（无图片）}'} 在中间位置
  - 结尾：最后1张图片 {{IMAGE_${imageCount}}} 在 ## 总结建议 之前
- 图片占位符单独一行，前后各空一行` : imageSource === 'upload' ? `- 预留图片位置，用户将上传图片（用 {{IMAGE_PLACEHOLDER}} 表示）` : `- 不生成图片，纯文字文章`}
